...
function Header({ staff, onLogout }) {
  const roleLabel = { foh: 'FOH', senior_foh: 'Sr. FOH', admin: 'Manager', hq: 'HQ' }
  const roleClass = { foh: 'role-foh', senior_foh: 'role-foh', admin: 'role-admin', hq: 'role-hq' }
  return (
...
function ManagerNav({ tab, setTab, isHQ }) {
  const tabs = [
...
export default function App() {
  ...
  if (isAdmin()) {
    const renderTab = () => {
      switch (managerTab) {
        case 'dashboard': return <Dashboard onNavigate={setManagerTab} />
        ...
        default:          return <Dashboard onNavigate={setManagerTab} />
      }
    }
    return (
      <div className="app-shell">
        ...
        <ManagerNav tab={managerTab} setTab={setManagerTab} isHQ={isHQ()} />
      </div>
    )
  }
